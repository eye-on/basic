...
FootballVisionConfig default_vision_config_for_sensor() {
  ...
  config.camera.fx = 509.2;
  config.camera.fy = 508.7;
  config.camera.cx = 320.4;
  config.camera.cy = 241.1;
  config.camera.k1 = -0.102;
  config.camera.k2 = 0.031;
  config.camera.p1 = 0.0;
  config.camera.p2 = -0.001;
...
class FootballRobot final : public basic::app::Robot {
 public:
  ...
  basic::vision::EstimateResult submit_yolo_detection(const YoloDetection& detection) {
    ...
    state_.vision.last_blob_detection.origin_x_px =
        static_cast<int>(detection.bbox_px.x);
    state_.vision.last_blob_detection.origin_y_px =
        static_cast<int>(detection.bbox_px.y);
    ...
    state_.vision.last_blob_detection.width_px =
        static_cast<int>(detection.bbox_px.width);
    state_.vision.last_blob_detection.height_px =
        static_cast<int>(detection.bbox_px.height);
 ...
 private:
  ...
  void show_vision_status() {
    const FootballVisionState& vision = state_.vision;
    const bool online =
        hardware_.external_vision.online(hardware_.brain.timer(vex::timeUnits::msec));

    hardware_.controller.Screen.setCursor(1, 1);
    hardware_.controller.Screen.print(
        "A:%s C:%-6s",
        auto_mode_enabled_ ? "ON " : "OFF",
        target_color_name(vision.target_color));
    ...
    hardware_.controller.Screen.print(
        ...
        static_cast<int>(vision.last_detection.bbox_px.x +
                         vision.last_detection.bbox_px.width * 0.5),
        static_cast<int>(vision.last_detection.bbox_px.y +
                         vision.last_detection.bbox_px.height * 0.5),
        static_cast<int>(vision.last_detection.bbox_px.x));
  }
