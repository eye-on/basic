#ifndef FIRST_ORDER_ADRC_TD_HPP_
#define FIRST_ORDER_ADRC_TD_HPP_
...
class TD {
 public:
  ...
  Output update(double v) {
    ...
    const double fh = clamp_value(fh_raw, -cfg_.max_acc, cfg_.max_acc);

    state_.x1 += cfg_.h * state_.x2;
    state_.x2 += cfg_.h * fh;
    state_.x2 = clamp_value(state_.x2, -cfg_.max_vel, cfg_.max_vel);
